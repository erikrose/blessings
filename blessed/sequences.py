...
def init_sequence_patterns(term):
    ...
    For debugging, complimentary lists of these sequence matching pattern
    values prior to compilation are attached as attributes ``_will_move``,
    ``_wont_move``, ``_cuf``,  ``_cub``.
    """
...
#        term.vpa = screen_vpa
    ...
    term._will_move = _merge_sequences(will_move_seqs)
    term._wont_move = _merge_sequences(wont_move_seqs)
    ...
    term._cuf = bnc(cap='cuf', optional=True)
    ...
    term._cub = bnc(cap='cub', optional=True)
