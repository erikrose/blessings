...
def _read_until_semaphore(fd, semaphore=RECV_SEMAPHORE,
    ...
    # line feed, it is only for conveience that \\n does both.
    #
    ...
    # continious integration servers such as Travis.
    outp = unicode()
...
#    signal.alarm(timeout)
    while not outp.startswith(semaphore):
        try:
            ...
            raise RuntimeError("Timeout for semaphore (output:%r)" % (outp,))
        if not _exc:  # bsd EOF
            break
...
def _read_until_eof(fd, encoding='utf8'):
...
def test_inkey_0s_input():
    ...
    if pid is 0:
        ...
        term = TestTerminal()
        _read_until_semaphore(sys.__stdin__.fileno(), semaphore=SEMAPHORE)
    ...
    with echo_off(master_fd):
        ...
        _read_until_semaphore(master_fd)
        stime = time.time()
...
def test_esc_delay_035():
    ...
    with echo_off(master_fd):
        ...
        key_name, duration_ms = _read_until_eof(master_fd).split()
    pid, status = os.waitpid(pid, 0)
